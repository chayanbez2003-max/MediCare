...
// this function will remove duplicate slot and return slot filtered time
...
/**
 * Controller to register a new doctor.
 * 
 * Responsibilities:
 ...
 * - Return sanitized doctor data to client
 */
...
export async function createDoctor(req, res){
  try{
    ...
    if(req.file?.path){
      ...
      imageUrl = uploaded?.sequre_Url || uploaded?.url || imageUrl;
    ...
    console.error("Faild to create the doctor profile:",err);
    return res
    .status(500)
    .json({
      success:false,
...
export async function toggleAvailability(req,res){
  try {
   ...
   if(typeof doc.availability === "boolean") doc.availability = !doc.availability ;
    ...
    console.error("toggleDoctorAvaility Error:",err);
    return res.status(500).json({
      success:false,
      message:"Server error"
    })
